...
{
    public class SingleMenuAttributeFilter : ActionFilterAttribute
    {
        ...
        public SingleMenuAttributeFilter(IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, ApplicationDbContext db)
        {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            int menuID = 0;
             ...
             menuID = _unitOfWork.Menu.GetAll(x => x.IsActive == true && x.AreaName == Area && x.ControllerName == Controller && x.MenuUnder == 0 && x.ActionName == Action && x.IsSingle == true).Select(x => x.Id).FirstOrDefault();
            if(menuID == 0)
            {
              menuID = _unitOfWork.Menu.GetAll(x => x.IsActive == true && x.AreaName == Area && x.ControllerName == Controller && x.MenuUnder == 0 && x.IsSingle == true).Select(x => x.Id).FirstOrDefault();
            ...
            {
                int orderby = 10;
                if(Controller.ToLower().Contains("dashboard"))
                {
                    orderby = 1; 
                }
                Menu menu = new Menu();
                ...
                menu.IsSingle = true;
                menu.MenuURL = "/" + Area + "/" + Controller + "/" + Action;
                menu.MenuName = Controller;
                menu.IsActive = true;
                menu.CreatedBy = userId;
                menu.CreatedDate = DateTime.Now;
                menu.MenuOrder = orderby;
                menu.MenuIcon = "fas fa-cubes";
