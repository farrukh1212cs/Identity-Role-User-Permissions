...
{
    public class ApiAuthroization : ActionFilterAttribute
    {
        ...
        public ApiAuthroization(IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, ApplicationDbContext db)
        {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            int menuID = 0;
             ...
             menuID = _unitOfWork.Menu.GetAll(x => x.IsActive == true && x.AreaName == Area && x.ControllerName == Controller && x.MenuUnder == 0 && x.ActionName == Action && x.API == true).Select(x => x.Id).FirstOrDefault();
            if(menuID == 0)
            {
          ...
          //  var userIds = User.FindFirst(ClaimTypes.NameIdentifier).Value.ToString();
            //get login UserID
           ...
           // var allMenusInRole = _unitOfWork.RoleRight.GetAll(x => roleas.Contains(x.Role_Id) && x.MenuId == menuID && x.IsActive == true).ToList();
            ...
            //{
                ...
                isInRoleAllowed = _unitOfWork.RoleRightAction.GetAll(x => roleas.Contains(x.Role_Id) && x.ActionId == 0 && x.IsActive == true).Count();
            //}
           ...
           // var allMenusToUser = _unitOfWork.UserRightAction.GetAll(x => x.Userid == userId && x.MenuId == menuID && x.IsActive == true).ToList();
            ...
            //    }
                ...
                isInUserAllowed = _unitOfWork.UserRightAction.GetAll(x => x.MenuId == menuID && x.ActionId == 0 && x.IsActive == true && x.Userid == userId).Count();
