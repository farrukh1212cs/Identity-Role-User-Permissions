...
namespace Insurance.ViewComponents
{
     public class SideBarViewComponent : ViewComponent
     {
        ...
        public SideBarViewComponent(IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor, ApplicationDbContext db, UserManager<ApplicationUser> userManager)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (IsSuperUser)
            {
                 Menus = _unitOfWork.Menu.GetAll(x => x.IsActive == true && x.IsSingle == false && x.API == false).ToList();
            }
            else
            {
                ...
                foreach (var item in RoleMenusIDS)
                {
                    ...
                    int parentID = _unitOfWork.Menu.GetAll(x => x.Id == Convert.ToInt32(item) && x.IsActive == true && x.API == false).Select(x => x.MenuUnder).FirstOrDefault();                
                ...
                Menus = _unitOfWork.Menu.GetAll(x => NewIDS.Contains(x.Id) && x.IsActive == true && x.API == false).ToList();
